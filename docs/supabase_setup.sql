-- 1. Limpeza total para sincronia (CUIDADO: apaga dados existentes)
...
CREATE TABLE public.profiles (
  ...
  full_name TEXT,
  avatar_url TEXT,
  role user_role DEFAULT 'free' NOT NULL, -- COLUNA DE TIPO DE USUÁRIO
...
-- 5. Função de Segurança para verificar Admin (Evita recursão RLS)
...
CREATE POLICY "Admins gerenciam tudo em perfis" ON public.profiles FOR ALL USING (public.is_admin());
...
CREATE POLICY "Usuários editam próprios cartões" ON public.cards FOR UPDATE USING (auth.uid() = user_id OR public.is_admin());
CREATE POLICY "Usuários deletam próprios cartões" ON public.cards FOR DELETE USING (auth.uid() = user_id OR public.is_admin());
